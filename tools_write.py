...
LOVABLE_API_BASE = os.getenv("LOVABLE_API_BASE", "https://api.lovable.dev/v1").rstrip("/")

LOVABLE_PROJECT_ALLOWLIST = {
    p.strip()
    for p in os.getenv("LOVABLE_PROJECT_ALLOWLIST", "").split(",")
    if p.strip()
}
...
# -------------------------------------------------------------------- lovable
# NOTE: verify these endpoint paths against Lovable's current API docs before first
# use. The tool names and arguments mirror the Lovable MCP server, so if the REST
# shape differs you only need to change the two _lovable_* helpers below.


def _lv_headers():
    if not LOVABLE_API_KEY:
        raise WriteRefused("LOVABLE_API_KEY is not set.")
    return {
...
def _check_project(project_id):
    if not project_id:
        raise WriteRefused("project_id is required.")
    if not LOVABLE_PROJECT_ALLOWLIST:
        raise WriteRefused("LOVABLE_PROJECT_ALLOWLIST is empty — no project is writable.")
    if project_id not in LOVABLE_PROJECT_ALLOWLIST:
        raise WriteRefused(f"Project '{project_id}' is not on the write allowlist.")


def tool_lovable_read_file(project_id, path):
    """Read-only. Not registered as a write tool."""
    r = requests.get(
        f"{LOVABLE_API_BASE}/projects/{project_id}/files/{path}",
        headers=_lv_headers(),
        timeout=TIMEOUT,
    )
    if r.status_code != 200:
        raise WriteRefused(f"Read failed: {r.status_code} {r.text[:300]}")
    return r.text[:8000]


def tool_lovable_send_message(project_id, message, plan_mode = False):
    """
    Send a build instruction to a Lovable project. This changes a live product and
    spends credits — hence confirmation. plan_mode=True makes Lovable plan without
    editing code, which is the safer first move on anything non-trivial.
    """
    _check_project(project_id)
    if not message.strip():
        raise WriteRefused("An empty message would burn credits for nothing.")

    r = requests.post(
        f"{LOVABLE_API_BASE}/projects/{project_id}/messages",
        headers=_lv_headers(),
        json={"message": message, "plan_mode": plan_mode},
        timeout=120,
    )
    if r.status_code not in (200, 201, 202):
        raise WriteRefused(f"Lovable message failed: {r.status_code} {r.text[:300]}")
    return f"Sent to Lovable project {project_id}.\n{r.text[:2000]}"
...
WRITE_TOOL_FUNCTIONS = {
    ...
    "lovable_send_message": tool_lovable_send_message,
}

READ_TOOL_FUNCTIONS = {
    "lovable_read_file": tool_lovable_read_file,
}
...
WRITE_TOOL_SCHEMAS = [
    {
        ...
        "function": {
            ...
            "description": (
                "Create a new branch in one of Dal's repos. Requires Dal's confirmation "
                "before it runs. Branch names should be brooksy/<short-description>."
            ),
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "Create or update one file on a non-default branch. Read the file first. "
                "Never targets main/master/production. Requires Dal's confirmation."
            ),
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            "name": "github_open_pr",
            "description": (
                "Open a pull request. Body should cover what changed, why, what was not "
                "touched, and what Dal should test. Requires Dal's confirmation."
            ),
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "Insert one row into an allowlisted table. Requires Dal's confirmation. "
                "There is no delete tool and never will be."
            ),
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "Update exactly one row by primary key in an allowlisted table. Query the "
                "row first. No bulk updates. Requires Dal's confirmation."
            ),
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Read one file from a Lovable project. Read-only, runs immediately.",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "Send a build instruction to a Lovable project. Changes a live product and "
                "spends credits. Use plan_mode=true first for anything non-trivial. "
                "Requires Dal's confirmation."
            ),
            "parameters": {
                "type": "object",
                "properties": {
